...
void Document::add_page(
    ...
    if (this->m_pages.find(page_id) != this->m_pages.end()) {
        common::warn("(Document::add_page) page with this ID already "
...
void Document::set_curr_page(const std::string& page_id) {
    if (this->m_pages.find(page_id) == this->m_pages.end()) {
        common::warn("(Document::set_curr_page) page with this ID does not "
                     "exist, doing nothing");
        return;
    }
...
void Document::render_curr_page(Document& doc, Context& ctx) {
    ...
    if (iter == this->m_pages.end()) {
        common::error("(Document::render_curr_page) page with this ID does "
